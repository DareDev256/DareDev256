...
### [UIVPG](https://github.com/DareDev256/Ultimate-Image-Video-Prompt-Generator) [![Live](https://img.shields.io/badge/Live-Try_It-10B981?style=flat-square)](https://prompts.tdotssolutionsz.com)
...
| **Viewport-aware scoring** | Signal Pulse auto-selects panels via deterministic scoring (scroll position × interaction signals × content adjacency). Rules layer is pure functions — testable, no hidden state |
...
<details>
<summary><strong>How the Passion Ecosystem Works</strong></summary>
...
| [UIVPG](https://github.com/DareDev256/Ultimate-Image-Video-Prompt-Generator) ⭐7 | Structured prompt builder for 5 AI image/video models — [**Live**](https://prompts.tdotssolutionsz.com) | TypeScript |
...
|---------|-------------|
| [red-team-arena](https://github.com/DareDev256/red-team-arena) | Adversarial LLM testing — [**Live**](https://red-team-arena.vercel.app) |
| [bias-buster](https://github.com/DareDev256/bias-buster) | Detect bias in LLM outputs — [**Live**](https://bias-buster-five.vercel.app) |
| [token-prophet](https://github.com/DareDev256/token-prophet) | Token prediction & cost optimization — [**Live**](https://token-prophet.vercel.app) |
...
> **UX Engineering:** Signal Pulse auto-selection engine (user-togglable) · AutoSelect dropdown pre-selection · Cinematic hero orchestration · SSE invalidation starvation
...
6. **SVG security** — blocks `<script>`, `<foreignObject>`, inline event handlers (`on*=`), non-fragment `url()`, `@import`, `@font-face`, and external `xlink:href` — enforces CWE-79/CWE-918 constraints documented in `signature.svg`
7. **Secret scanning** — word-bounded ERE regex (`\bsk-…{20+}`, `\bghp_…{36+}`, `\bAKIA…{16+}`, `\bBearer …{20+}`) with ERE-mode doc-line exclusion (backtick-delimited patterns require both opening and closing backticks to skip scanning); now also scans `signature.svg`
...
**Prerequisites:** A GitHub account and a GFM-compatible Markdown previewer (`grip`, VS Code, or push-to-branch).
...
|------|---------|-----------|--------|
| Daily Status | `<!-- DAILY_STATUS_START -->` / `<!-- DAILY_STATUS_END -->` | ~Daily | `Today: **N tasks** across **N repos** · **+N/-N lines** · N% success rate` |
| Showcase | `<!-- SHOWCASE_SECTION_START -->` / `<!-- SHOWCASE_SECTION_END -->` | After notable builds | `Tonight's build: [repo](url)` + summary + highlights + diff stats |
...
| CI validation | GitHub Actions runs [7 checks](#how-it-works) on every push/PR — structural integrity (line count, markers, version chain, SVG presence), SVG security enforcement (blocks XSS vectors), and word-bounded ERE secret scanning with backtick-delimited doc-line exclusion | [CWE-79](https://cwe.mitre.org/data/definitions/79.html), [CWE-798](https://cwe.mitre.org/data/definitions/798.html), [CWE-918](https://cwe.mitre.org/data/definitions/918.html), structural drift |
